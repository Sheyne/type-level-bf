pub use typenum::{True, False};

pub trait IsNonZero {
    type Result;
}

impl<T> IsNonZero for T where T : typenum::IsNotEqual<Zero> {
...
impl<T> IncrNat for T where T : std::ops::Add<One>{
...
impl<T> DecrNat for T where T : std::ops::Sub<One>{
    type Result = <T as std::ops::Sub<One>>::Output;
}

use super::Make;

impl<T> Make<i8> for T where T : typenum::Integer
{
    fn make() -> i8 {
        T::to_i8()
    }
...
impl<T> Make<i32> for T where T : typenum::Integer
{
    fn make() -> i32 {
        T::to_i32()
    }
